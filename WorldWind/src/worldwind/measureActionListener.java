...
import java.awt.print.*;
    ...
    class measureActionListener implements ActionListener {
        ...
        public measureActionListener(WWJ parent, JPanel Pfrom, JPanel panel, JPanel Pto, JTextArea jta) {
            this.parent = parent;
            this.Pfrom = Pfrom;
            this.panel = panel;
            this.Pto = Pto;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            parent.ww.getView().goTo(new Position(parent.textFieldPoints(from.getText()).get(0).getLatlon(), 0), 3000e3);
            combined.add((parent.textFieldPoints(from.getText()).get(0)));
            for (int i = 0; i < panel.getComponentCount(); i++) {
                if (i % 3 == 1) {
                    if (!(parent.textFieldPoints(((JTextField) panel.getComponent(i)).getText()).isEmpty())) {
                        JComboBox jcbm = (JComboBox) panel.getComponent(i + 2);
                        jcbm.setVisible(false);
                        if (parent.textFieldPoints(((JTextField) panel.getComponent(i)).getText()).size() > 1) {
                            jcbm.removeAllItems();
                            java.util.List<PointOfInterest> each = parent.textFieldPoints(((JTextField) panel.getComponent(i)).getText());
                        ...
                        combined.add((parent.textFieldPoints((((JTextField) panel.getComponent(i)).getText())).get(0)));
                    } else {
                        continue;
                    }
                }
            ...
            if (parent.textFieldPoints(from.getText()).size() > 1) {
                java.util.List<PointOfInterest> each = parent.textFieldPoints(from.getText());
                for (PointOfInterest poi : each) {
                    jcbf.addItem(poi);
                }
            ...
            if (parent.textFieldPoints(to.getText()).size() > 1) {
                java.util.List<PointOfInterest> each = parent.textFieldPoints(to.getText());
                for (PointOfInterest poi : each) {
                    jcbt.addItem(poi);
                }
