...
/*
 * \class GenHTMLActionListener listener for generating HTML reports
 */
class GenHTMLActionListener implements ActionListener {
    ...
    public GenHTMLActionListener(WWJ parent,
            Vector<EarthquakeContentElement> tbContent,
            Vector<EarthquakeAttributes> eqattr) {
        this.parent = parent;
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    out.write("<script type=\"text/javascript\" "
                            ...
                            + "</script>\n");
                    out.write("<script type=\"text/javascript\">\n");
                    out.write("google.load('visualization', '1.1', "
                            + "{packages: ['controls']});\n");
                    out.write("</script>\n");
                    out.write("<script type=\"text/javascript\">");
                    out.write("function drawVisualization() {\n"
                            ...
                            + "['Title', 'Image', 'Summary', 'Position', "
                            + "'Magnitude', 'Details', 'Latitude', "
                            + "'Longitude', 'Elevation', 'Magnitude', "
                            + "'Location']\n");
                    if (tbContent.size() != 0) {
                        out.write(",\n");
                    }
                    for (int i = 0; i < tbContent.size(); i++) {
                        String location =
                                tbContent.elementAt(i).title.substring(
                                tbContent.elementAt(i).title.indexOf(",")
                                + 2, tbContent.elementAt(i).title.length());
                        out.write("['"
                                + tbContent.elementAt(i).title
                                + "', '"
                                + tbContent.elementAt(i).image
                                + "', '"
                                + tbContent.elementAt(i).summary
                                + "', '"
                                + tbContent.elementAt(i).position
                                + "', "
                                + tbContent.elementAt(i).magnitude
                                + ", '"
                                + tbContent.elementAt(i).details
                                + "'," + eqattr.elementAt(i).latitude
                                + ", "
                                + eqattr.elementAt(i).longitude
                                + ", "
                                + eqattr.elementAt(i).elevation
                                + ", "
                                + eqattr.elementAt(i).magnitude
                                + ", '"
                                + location
                                + "']");
                    ...
                    out.write("myView = "
                            ...
                            + "bind([stringFilter, latitude, longitude, "
                            + "elevation, magnitude], [table])."
                            + "draw(myView );\n"
                            + "}\n"
                            + "google."
